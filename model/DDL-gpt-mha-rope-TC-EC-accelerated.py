...
from .modules.input_embed import TritonInputEmbedShortConvExpander as InputEmbedShortConvExpander
...
class ResidualShortConvCompressor(_TemporalResidualShortConvCompressor):
    def __init__(self, config: Any) -> None:
        super().__init__(config, implementation="triton_required", module_name="DDL-gpt-mha-rope-TC-EC-accelerated")
...
class DeepDeltaResidualExpanded(nn.Module):
    def __init__(self, config: Any):
        ...
        if not self.beta_single_linear or self.v_constant or not self.v_sigmoid:
            raise ValueError(
                "DDL-gpt-mha-rope-TC-EC-accelerated requires ddl_beta_single_linear=True, "
    ...
    def forward(
        ...
        *,
        ...
        validate_expanded_delta_inputs(
            module_name="DDL-gpt-mha-rope-TC-EC-accelerated",
            x=x,
            k_in=k_in,
            v_in=v_in,
            context=context,
        ...
        if not (x.is_cuda and k_in.is_cuda and v_in.is_cuda and context.is_cuda):
            raise RuntimeError("DDL-gpt-mha-rope-TC-EC-accelerated requires CUDA tensors for fused Triton updates.")
...
class GPT(DecoderOnlyCausalLMPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        input_embed_has_state = int(getattr(self.input_embed, "kernel_size", 0)) > 1
        readout_shortconv = getattr(self.readout, "shortconv", None)
        readout_kernel_size = int(getattr(readout_shortconv, "kernel_size", 0)) if readout_shortconv is not None else 0
        readout_has_state = readout_kernel_size > 1
        readout_state_shape = (
            (
                batch_size,
                int(
        ...
        if past_key_values is not None:
            for layer_idx, layer_past in enumerate(past_key_values):
                ...
                first_layer_has_embed_state = layer_idx == 0 and input_embed_has_state
                layer_has_readout_state = readout_state_shape is not None and extra_state_count >= (
                ...
                if layer_has_readout_state:
                    assert readout_state_shape is not None
        ...
        for layer_idx, block in enumerate(transformer_blocks):
            if use_cache_flag or past_key_values is not None or attention_mask is not None or position_ids is not None:
                ...
                if use_cache_flag:
                    ...
                    if layer_idx == 0 and embed_state_out is not None:
                        present = (*present[:-1], embed_state_out, present[-1])
            ...
            if output_hidden_states_flag:
                ...
                x_hidden, layer_readout_state_out = cast(Any, self.readout).forward_with_past(
                    x,
        ...
        if output_hidden_states_flag and len(transformer_blocks) > 0:
            ...
            x_out, readout_state_out = cast(Any, self.readout).forward_with_past(
                x,
                past=readout_state,
                token_mask=token_mask,
            )
