...
from .modules.kv_shift import find_kv_cache_state_by_shape, keep_only_kv_shift_cache_states
...
class ResidualShortConvCompressor(_CcResidualShortConvCompressor):
    def __init__(self, config: Any) -> None:
        super().__init__(config, implementation="triton_required", module_name="DDL-gpt-mha-rope-CC-EC-accelerated")
...
class DeepDeltaResidualExpanded(nn.Module):
    def __init__(self, config: Any):
        ...
        if not self.beta_single_linear or self.v_constant or not self.v_sigmoid:
            raise ValueError(
                "DDL-gpt-mha-rope-CC-EC-accelerated requires ddl_beta_single_linear=True, "
    ...
    def forward(
        ...
        *,
        ...
        validate_expanded_delta_inputs(
            module_name="DDL-gpt-mha-rope-CC-EC-accelerated",
            x=x,
            k_in=k_in,
            v_in=v_in,
            context=context,
        ...
        if not (x.is_cuda and k_in.is_cuda and v_in.is_cuda and context.is_cuda):
            raise RuntimeError("DDL-gpt-mha-rope-CC-EC-accelerated requires CUDA tensors for fused Triton updates.")
