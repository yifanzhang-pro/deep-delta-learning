    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if past_key_values is not None:
            ...
            extras = list(past0[4:-1]) if len(past0) >= 6 else []
            if self.input_embed.kernel_size > 1 and extras:
                embed_state = extras[0]
                extras = extras[1:]
            if extras:
                readout_state = extras[0]
        ...
        if use_cache_flag:
            ...
            if present_key_values and (embed_state_out is not None or readout_state_out is not None):
                ...
                if readout_state_out is not None:
                    merged.append(readout_state_out)
                merged.append(past0[-1])
                present_key_values[0] = tuple(merged)
